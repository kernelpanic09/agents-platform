    ...
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          ...
          { icon: Zap, label: 'API Calls', value: costs?.byModel?.reduce((a, m) => a + m.calls, 0) || '--', color: '#3B82F6' },
          { icon: Clock, label: 'Avg Latency', value: latency?.stats?.[0] ? `${latency.stats[0].avg_ms}ms` : '--', color: '#F59E0B' },
          { icon: Activity, label: 'Models Used', value: costs?.byModel?.length || '--', color: '#8B5CF6' },
        ].map((card, i) => {
