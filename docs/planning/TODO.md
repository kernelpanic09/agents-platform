...
- **Deps:** concrete files/tables/features that must exist first. `none` means no in-repo blocker.
...
- [ ] Refactor `executor.js` stdout handling from string accumulation to a streaming callback / `EventEmitter` keyed by `runId` (final transcript still written to `runs.transcript` on completion). **[M · T1 · Deps: none]**
...
- [ ] In `parseClaudeJson()`, stop discarding `parsed.usage`; extract input/output token counts (fall back to a `chars/4` estimate with an `estimated` badge for older CLI output). **[S · T2 · Deps: none]**
...
- [ ] Add a `platform_settings` table (`key`, `value`, `value_type`, `description`, `editable_live`, `requires_restart`); env vars seed defaults, DB values override at read time. **[M · T4 · Deps: none]**
...
- [ ] Refactor `server/safety-prompt.js` from a single hardcoded `SAFETY_PREAMBLE` constant into `buildSafetyPreamble(policy)` / `policyToPrompt(policy)`. **[S · T3 · Deps: none]**
...
- [ ] Add an `api_keys` table (`key_hash`, `key_prefix`, `scopes` JSON, `rate_limit_rpm`, `last_used_at`, `revoked`); keys shown in full once on create (GitHub-PAT pattern). **[M · T7 · Deps: none]**
...
- [ ] Add a `webhooks` table (per-schedule tokens, `label`, `last_triggered_at`, `trigger_count`). **[S · T7 · Deps: none]**
...
- [ ] Persist the queue to a `job_queue` table (`run_id`, `attempt`, `max_attempts`, `next_attempt_at`, `status`, `locked_at`) so a pod restart doesn't silently drop queued/running jobs. **[M · T9 · Deps: none]**
...
- [ ] Add `notification_channels` + `schedule_notifications` tables; refactor the hardcoded `DISCORD_WEBHOOK_URL` into a DB-driven `NotificationRouter` (on-start / success / failure rules, optional `filter_expr`). **[M · T7 · Deps: none]**
...
- [ ] Add a `prompt_versions` table (`agent_id`, `version_tag`, `system_prompt`, `notes`, `created_at`); auto-snapshot the prior value on every `PUT /api/agents/:id` that changes `system_prompt`. **[M · T6 · Deps: none]**
...
- [ ] Add a `workflow_graph` JSON column to `schedules` (nodes = agents, edges carry a `data_key` + condition); add `graph` as a fourth mode alongside parallel/sequential/meeting. **[M · T5 · Deps: none]**
...
- [ ] Add a `compositions` table (named reusable agent teams: `agent_ids`, `mode`, `task_prompt_template`, `tags`); add "Save as Composition" on `ComposePage`. **[M · T5 · Deps: none]**
